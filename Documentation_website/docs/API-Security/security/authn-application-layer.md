...
| **scope** | OPTIONAL parameter which defines what resources or actions the client application is allowed to access when acting on its own behalf. It enables fine-grained control over permissions by specifying which capabilities the issued access token should include| 
